...
- [x] Restore a privileged manual-review trigger (REST/UI) that bypasses listener gating for admin force-runs.
- [x] Refactor diff collection to stream incrementally, reducing memory pressure for large pull requests.

## History & Metrics

- [x] Add database indexes for history queries (`REVIEW_START_TIME`, project/repo) to keep admin views responsive.
- [x] Capture richer state in review history (source/target commits, PR version) for accurate idempotency checks.
